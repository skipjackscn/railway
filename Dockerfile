...
RUN set -eux; \
    ...
    if ! curl -fsSL --retry 3 --retry-delay 2 -o /tmp/et.zip "$URL"; then \
        ...
        URL="$(curl -fsSL https://api.github.com/repos/EasyTier/EasyTier/releases/latest | grep -o "https://[^\"]*linux-${ET_ARCH}-[^\"]*\.zip" | head -n1)"; \
...
# 所以必须删除后直接用重定向生成，同时给 /var/lib/dbus/machine-id 做软链。
RUN set -eux; \
